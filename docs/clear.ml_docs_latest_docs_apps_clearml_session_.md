---
url: "https://clear.ml/docs/latest/docs/apps/clearml_session/"
title: "ClearML Session CLI | ClearML"
---

[Skip to main content](https://clear.ml/docs/latest/docs/apps/clearml_session/#__docusaurus_skipToContent_fallback)
...
![JupyterLab session](https://clear.ml/docs/latest/assets/images/session_jupyter-efda41fd8efe340f8996963cd4c8a61b.png)

VS Code Window

![VS Code session](https://clear.ml/docs/latest/assets/images/session_vs_code-db6f4052b3e5a033556f724e679bc77e.png)

## Prerequisites [​](https://clear.ml/docs/latest/docs/apps/clearml_session/\#prerequisites "Direct link to Prerequisites")

- `clearml` installed and configured. See [ClearML Setup](https://clear.ml/docs/latest/docs/clearml_sdk/clearml_sdk_setup) for details.
- At least one `clearml-agent` running on a remote host. See [installation](https://clear.ml/docs/latest/docs/clearml_agent/clearml_agent_deployment_bare_metal#installation) for details.
...
## Launching ClearML Session [​](https://clear.ml/docs/latest/docs/apps/clearml_session/\#launching-clearml-session "Direct link to Launching ClearML Session")
...
You can add flags to set a container image, the remote SSH port, JupyterLab/VS Code versions, and more. See [CLI options](https://clear.ml/docs/latest/docs/apps/clearml_session/#command-line-options)
for all configuration options.

`clearml-session` creates a new [Task](https://clear.ml/docs/latest/docs/fundamentals/task) that is responsible for setting up the SSH and
JupyterLab/VS Code environment according to your specifications on the host machine.

3. Follow the `clearml-session` setup wizard:
...
4. The session Task is enqueued in the selected queue, and a ClearML Agent pulls and executes it. The agent downloads the appropriate IDE(s) and
launches it.

5. Once the agent finishes the initial setup of the interactive Task, the local `clearml-session` connects to the host
...
6. The CLI outputs access links to the remote JupyterLab and VS Code sessions:
...
7. Now start working on the code as if you're running on the target machine itself!


## Re-launching and Shutting Down Sessions [​](https://clear.ml/docs/latest/docs/apps/clearml_session/\#re-launching-and-shutting-down-sessions "Direct link to Re-launching and Shutting Down Sessions")
...
## Connecting to an Existing Session [​](https://clear.ml/docs/latest/docs/apps/clearml_session/\#connecting-to-an-existing-session "Direct link to Connecting to an Existing Session")
...
2. Copy the unique ID by clicking the `ID` button in the task page's header.
3. Run the following command: `clearml-session --attach <session_id>`.
4. Click on the JupyterLab / VS Code link that is outputted, or connect directly to the SSH session

## Features [​](https://clear.ml/docs/latest/docs/apps/clearml_session/\#features "Direct link to Features")

### Running in a Container [​](https://clear.ml/docs/latest/docs/apps/clearml_session/\#running-in-a-container "Direct link to Running in a Container")

To run a session inside a container, use the `--docker` flag and enter the image to use in the interactive
session.

### Kubernetes Support [​](https://clear.ml/docs/latest/docs/apps/clearml_session/\#kubernetes-support "Direct link to Kubernetes Support")
...
For more information, see [Kubernetes](https://clear.ml/docs/latest/docs/clearml_agent/clearml_agent_deployment_k8s).

### Installing Requirements [​](https://clear.ml/docs/latest/docs/apps/clearml_session/\#installing-requirements "Direct link to Installing Requirements")
...
### Accessing a Git Repository [​](https://clear.ml/docs/latest/docs/apps/clearml_session/\#accessing-a-git-repository "Direct link to Accessing a Git Repository")
...
### Uploading Local Files to Remote Session [​](https://clear.ml/docs/latest/docs/apps/clearml_session/\#uploading-local-files-to-remote-session "Direct link to Uploading Local Files to Remote Session")
...
development machines and remote machines with persistent workspace synchronization. See [Storing and Synchronizing Workspace](https://clear.ml/docs/latest/docs/apps/clearml_session/#storing-and-synchronizing-workspace).

### Starting a Debugging Session [​](https://clear.ml/docs/latest/docs/apps/clearml_session/\#starting-a-debugging-session "Direct link to Starting a Debugging Session")
...
2. Copy the unique ID by clicking the `ID` button in the task page's header.
3. Enter the following command: `clearml-session --debugging-session <task_id>`
4. Click on the JupyterLab / VS Code link, or connect directly to the SSH session.
5. In JupyterLab / VS Code, access the task's repository in the `environment/task_repository` folder.

### Storing and Synchronizing Workspace [​](https://clear.ml/docs/latest/docs/apps/clearml_session/\#storing-and-synchronizing-workspace "Direct link to Storing and Synchronizing Workspace")
...
## Command Line Options [​](https://clear.ml/docs/latest/docs/apps/clearml_session/\#command-line-options "Direct link to Command Line Options")
...
| `--queue-excluded-tag` | The queue option list will exclude queues with specified tags. See the `tags` parameter in the [`queues.create`](https://clear.ml/docs/latest/docs/references/api/queues#post-queuescreate) API call. | `none` |
| `--queue-include-tag` | The queue option list will include only queues with specified tags. See the `tags` parameter in the [`queues.create`](https://clear.ml/docs/latest/docs/references/api/queues#post-queuescreate) API call. | `none` |
...
- [Prerequisites](https://clear.ml/docs/latest/docs/apps/clearml_session/#prerequisites)
- [Launching ClearML Session](https://clear.ml/docs/latest/docs/apps/clearml_session/#launching-clearml-session)
- [Re-launching and Shutting Down Sessions](https://clear.ml/docs/latest/docs/apps/clearml_session/#re-launching-and-shutting-down-sessions)
- [Connecting to an Existing Session](https://clear.ml/docs/latest/docs/apps/clearml_session/#connecting-to-an-existing-session)
- [Features](https://clear.ml/docs/latest/docs/apps/clearml_session/#features)
  - [Running in a Container](https://clear.ml/docs/latest/docs/apps/clearml_session/#running-in-a-container)
  - [Kubernetes Support](https://clear.ml/docs/latest/docs/apps/clearml_session/#kubernetes-support)
  - [Installing Requirements](https://clear.ml/docs/latest/docs/apps/clearml_session/#installing-requirements)
  - [Accessing a Git Repository](https://clear.ml/docs/latest/docs/apps/clearml_session/#accessing-a-git-repository)
  - [Uploading Local Files to Remote Session](https://clear.ml/docs/latest/docs/apps/clearml_session/#uploading-local-files-to-remote-session)
  - [Starting a Debugging Session](https://clear.ml/docs/latest/docs/apps/clearml_session/#starting-a-debugging-session)
  - [Storing and Synchronizing Workspace](https://clear.ml/docs/latest/docs/apps/clearml_session/#storing-and-synchronizing-workspace)
- [Command Line Options](https://clear.ml/docs/latest/docs/apps/clearml_session/#command-line-options)